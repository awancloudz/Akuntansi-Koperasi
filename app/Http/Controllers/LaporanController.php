...
class LaporanController extends Controller
{
    ...
    public function getPdf_shu(){
        ...
        $pdf = PDF::loadView('laporan.cetakshu',compact('daftarshu_','pinjaman','provisi','beban','gaji','penyusutan','pemakaian','bruto','netto','operasi','shutotal'))->setPaper('a4','portrait');
        return $pdf->stream();
    }
    public function getPdf_aruskas(){
        if(Auth::check()){
        $iduser = Auth::user()->id;
        }
        //Tanggal
        ...
        $pdf = PDF::loadView('laporan.cetakaruskas',compact('daftararuskas','piutang','hutangpendek','pemakaian','penyusutan','totaloperasi','perlengkapan','peralatan','totalinvestasi','hutangbank','simpananpokok','simpananwajib','totalpembiayaan','kasakhir','shutotal'))->setPaper('a4','portrait');
        return $pdf->stream();
    }
    public function getPdf_neraca(){
        if(Auth::check()){
        $iduser = Auth::user()->id;
        }
        //Tanggal
        ...
        $pdf = PDF::loadView('laporan.cetakneraca',compact('daftarneraca','aktivalancar','aktivatetap','totalaktiva','hutanglancar','hutangjangkapanjang','ekuitas','totalpasiva','kas','piutang','pemakaian','penyusutan','peralatan','hutangusaha','simpanansukarela','hutangbunga','hutangbank','simpananpokok','simpananwajib','shutotal'))->setPaper('a4','portrait');
        return $pdf->stream();
    }
    public function getPdf_simpanan(){
        if(Auth::check()){
        $iduser = Auth::user()->id;
        }
        //Tanggal
        ...
        $daftar = TransaksiSimpanan::whereBetween('tanggal', [$awalbulanini, $akhirbulanini])->get();
        $daftarsimpanan = $daftar->where('id_users',$iduser);
        $pdf = PDF::loadView('laporan.cetaksimpanan',compact('daftarsimpanan'))->setPaper('a4','portrait');
        return $pdf->stream();
    }
    public function getPdf_pinjaman(){
        if(Auth::check()){
        $iduser = Auth::user()->id;
        }
        //Tanggal
        ...
        $daftar = TransaksiPinjaman::whereBetween('tanggal', [$awalbulanini, $akhirbulanini])->get();
        $daftarpinjaman = $daftar->where('id_users',$iduser);
        $pdf = PDF::loadView('laporan.cetakpinjaman',compact('daftarpinjaman'))->setPaper('a4','portrait');
        return $pdf->stream();
    }
    public function getPdf_umum(){
        if(Auth::check()){
        $iduser = Auth::user()->id;
        }
        //Tanggal
        ...
        $daftar = TransaksiUmum::whereBetween('tanggal', [$awalbulanini, $akhirbulanini])->get();
        $daftarumum = $daftar->where('id_users',$iduser);
        $pdf = PDF::loadView('laporan.cetakumum',compact('daftarumum'))->setPaper('a4','portrait');
        return $pdf->stream();
    }
}
